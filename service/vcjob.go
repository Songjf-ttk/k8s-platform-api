...
// // 定义列表的返回内容,Items是vcjob元素列表,Total是元素数量
// type VcjobsResp struct {
// 	Total int          			`json:"total"`
// 	Items []volcanov1alpha1.Job `json:"items"`
// }
...
// 	//context.TODO()  用于声明一个空的context上下文,用于List方法内设置这个请求超时
// 	//metav1.ListOptions{} 用于过滤List数据,如label,field等
// 	vcjobList, err := vc.ClientSet.BatchV1alpha1().Jobs(namespace).List(context.TODO(), metav1.ListOptions{})
// 	if err != nil {
// 		klog.Error("Failed to get Vcjob list,", err)
// 		return nil, errors.New("failed to get Vcjob list," + err.Error())
// 	}

// 	// Assemble the dataSelector structure
// 	selectableData := &dataSelector{
// 		GenericDataList: vc.toCells(vcjobList.Items),
// 		DataSelect: &DataSelectQuery{
// 			Filter: &FilterQuery{Name: filterName},
...
// 	fmt.Printf("Vcjob total: %d\n", total)

// 	// Convert DataCell type back to Vcjob
// 	vcjobs := vc.fromCells(data.GenericDataList)
// 	return &VcjobResp{
// 		Items: vcjobs,
// 		Total: total,
// 	}, nil
...
// // 获取Pod内容器日志
// func (vc *vcjob) GetVcjobLog(containerName string, podName string, namespace string) (log string, err error) {
// 	//设置日志配置,容器名,获取内容的配置
